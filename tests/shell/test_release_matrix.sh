...
PUBLISH="$REPO_ROOT/.github/scripts/publish-release.sh"
TEST_TMP=$(mktemp -d "${TMPDIR:-/tmp}/liquid-formula-matrix-test.XXXXXX") || exit 1
...
assert_file_exists "$PUBLISH" "SHA-bound release publisher exists"
...
GOMOD="$REPO_ROOT/openwrt-feed/liquid-formula/src/go.mod"
...
assert_command_success "release publisher has valid shell syntax" sh -n "$PUBLISH"
...
printf 'main\n' > "$TEST_TMP/arts/pkg-a/liquid-formula_1.6.8_x86_64_openwrt-25.12.apk"
assert_command_failure "collector rejects a release with no LuCI package" \
	...
	--main liquid-formula --luci luci-app-liquid-formula

printf 'main\n' > "$TEST_TMP/good/pkg-a/liquid-formula_1.6.8_x86_64_openwrt-25.12.apk"
printf 'luci\n' > "$TEST_TMP/good/pkg-a/luci-app-liquid-formula_1.6.8_all_openwrt-25.12.apk"
...
assert_command_success "collector accepts a complete release set" \
	...
	--main liquid-formula --luci luci-app-liquid-formula
assert_file_exists "$TEST_TMP/goodout/liquid-formula_1.6.8_x86_64_openwrt-25.12.apk" "collector copies the service package"
assert_file_exists "$TEST_TMP/goodout/luci-app-liquid-formula_1.6.8_all_openwrt-25.12.apk" "collector copies the LuCI package"
assert_file_not_exists "$TEST_TMP/goodout/BUILD_INFO_x86_64.txt" "collector leaves per-job build info out of the release"

# --- workflow wiring ----------------------------------------------------------

# 发布产物的身份必须可信: 版本、包名、配对完整性任何一环松了, 发出去的
# release 就可能是混合来源的。
assert_contains "$WORKFLOW" 'does not match PKG_VERSION' "a tag that disagrees with PKG_VERSION fails the build"
assert_contains "$WORKFLOW" 'PKG_VERSION mismatch' "the LuCI package version must match the service package"
assert_contains "$PUBLISH" 'repos/\$\{GITHUB_REPOSITORY\}/commits/tags/\$\{tag\}' "existing tags are resolved to commits before publishing"
assert_contains "$PUBLISH" '\[ "\$tag_sha" != "\$expected_sha" \]' "a tag at another commit stops publication"
assert_contains "$PUBLISH" '--target "\$expected_sha"' "a new release tag points at the commit that was built"
assert_contains "$WORKFLOW" 'group: release-\$\{\{ github\.workflow \}\}-\$\{\{ needs\.test\.outputs\.pkg_version \}\}' "release jobs from different refs share a version-level lock"
assert_contains "$WORKFLOW" 'queue: max' "pending web uploads and releases remain queued"
assert_contains "$WORKFLOW" 'requested tag .* does not match PKG_VERSION' "a manually entered tag must match the source version"
assert_contains "$COLLECT" 'BUILD_MANIFEST' "the release records the exact OpenWrt point release it was built against"
assert_contains "$BUILD_ACTION" '\[-_\]\[0-9\]' "package globs anchor on the version so the DPI build is not mistaken for the main one"

# 行为验证: 缺 LuCI 包时必须拒绝发布 —— 那个 OpenWrt 版本的用户就没界面可装。
PAIR_IN="$TEST_TMP/pair-in/job"
mkdir -p "$PAIR_IN"
touch "$PAIR_IN/liquid-formula_9.9.9_x86_64_openwrt-25.12.apk"
assert_command_failure "collector rejects a release with no LuCI package" \
	python3 "$COLLECT" --input "$TEST_TMP/pair-in" --output "$TEST_TMP/pair-out" \
	--main liquid-formula --luci luci-app-liquid-formula

touch "$PAIR_IN/luci-app-liquid-formula_9.9.9_all_openwrt-25.12.apk"
assert_command_success "collector accepts a complete service/LuCI set" \
	python3 "$COLLECT" --input "$TEST_TMP/pair-in" --output "$TEST_TMP/pair-out2" \
	--main liquid-formula --luci luci-app-liquid-formula
...
assert_contains "$WORKFLOW" 'publish-release\.sh' "workflow delegates publication to the SHA-bound helper"
assert_contains "$PUBLISH" 'gh release (create|upload)' "release helper publishes with the GitHub CLI"
