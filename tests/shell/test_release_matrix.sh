...
assert_contains "$WORKFLOW" 'DPI_PACKAGE' "the DPI package version is checked alongside the others"
assert_contains "$BUILD_ACTION" '\[-_\]\[0-9\]' "package globs anchor on the version so the DPI build is not mistaken for the main one"
assert_contains "$COLLECT" 'present but' "the collector rejects an unpaired service/DPI set"

# 行为验证: 缺一个 DPI 包时必须拒绝发布。
...
assert_command_failure "collector rejects a service package with no matching DPI package" \
	python3 "$COLLECT" --input "$TEST_TMP/pair-in" --output "$TEST_TMP/pair-out" \
	--main liquid-formula --luci luci-app-liquid-formula --dpi liquid-formula-dpi

touch "$PAIR_IN/liquid-formula-dpi_9.9.9_x86_64_openwrt-25.12.apk"
assert_command_success "collector accepts a complete service/DPI/LuCI set" \
