...
# momo bypass 开关。掩码必须写死在预设里: momo 解析 bypass_fwmark 时不带 "/"
# 则掩码默认 0xFFFFFFFF, 生成 "mark & 0xffffffff == 0x8000", 而包经 momo 打标后
# 是组合值(0x8080), 不带掩码就永远不命中 —— 一个静默失效的坑。
ACL="$REPO_ROOT/openwrt-feed/luci-app-liquid-formula/root/usr/share/rpcd/acl.d/luci-app-liquid-formula.json"
...
assert_contains "$OVERVIEW" "'0x8000/0x8000'" "FakeHTTP bypass mark carries its mask"
assert_contains "$OVERVIEW" "'0x10000/0x10000'" "FakeSIP bypass mark carries its mask"
...
assert_not_contains "$OVERVIEW" "'0x8000'," "no maskless FakeHTTP mark can reach momo"
