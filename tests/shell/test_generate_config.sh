...
UA_PROBE="$REPO_ROOT/tools/probe-subscription.sh"
...
assert_fixed "$SBF_CONFIG_OUT" "  write_timeout: 150" "budgets the write timeout for the node fetch plus every enabled template"
...
assert_contains "$YAML_EXAMPLE" 'write_timeout:[[:space:]]+150' "keeps the example HTTP timeout above the subscription timeout"
assert_contains "$UCI_DEFAULTS" "option[[:space:]]+user_agent[[:space:]]+'v2rayN/7\.24\.4'" "fresh UCI defaults to a current v2rayN User-Agent"
assert_contains "$YAML_EXAMPLE" "user_agent:[[:space:]]+'v2rayN/7\.24\.4'" "example config uses the current v2rayN User-Agent"
assert_contains "$UA_PROBE" '^UA_LIST=' "ships the maintained User-Agent probe list"
for maintained_ua in \
	'v2rayN/7.24.4' 'v2rayNG/2.2.6' 'sing-box 1.13.15' \
	'SFI/1.13.15 \(sing-box 1.13.15\)' 'SFA/1.13.15 \(sing-box 1.13.15\)' \
	'SFM/1.13.15 \(sing-box 1.13.15\)' 'Karing/1.2.23.2605'; do
	assert_contains "$UA_PROBE" "$maintained_ua" "probe includes maintained User-Agent: $maintained_ua"
done
assert_not_contains "$UA_PROBE" 'v2rayN/7\.0\.0|sing-box 1\.11\.0|mihomo/1\.19\.0|Clash/v1\.18\.0' "probe removes outdated or format-mismatched presets"
...
assert_fixed "$SBF_CONFIG_OUT" "write_timeout: 40" "derives the lower HTTP timeout budget"
...
assert_fixed "$SBF_CONFIG_OUT" "write_timeout: 1230" "derives the upper HTTP timeout budget"

# 刷新是串行的, 所以每多一个启用的模板就要多一份 subscription_timeout。
reset_config
export UCI_main_enabled=1 UCI_main_subscription_url='https://provider.example/sub'
...
assert_fixed "$SBF_CONFIG_OUT" "write_timeout: 210" "the write budget grows with each enabled template"
...
assert_fixed "$SBF_CONFIG_OUT" "write_timeout: 150" "a disabled template costs no write budget"
...
assert_fixed "$SBF_CONFIG_OUT" "  url: 'https://provider.example/sub?token=complete-secret'" "passes the subscription URL through untouched"
...
for valid_url in \
	...
	reset_config
	export UCI_main_enabled=1 UCI_main_subscription_url="$valid_url"
	expect_generator_success "accepts backend-valid subscription URL: $valid_url"
	assert_fixed "$SBF_CONFIG_OUT" "  url: '$valid_url'" "preserves backend-valid subscription URL bytes: $valid_url"
done
