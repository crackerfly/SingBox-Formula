...
assert_make_top_level_contains \
	"$PACKAGE_MAKEFILE" \
	'^[[:space:]]*PKG_VERSION[[:space:]]*:=[[:space:]]*1\.5\.0[[:space:]]*$' \
	"sets package version 1.5.0 in active top-level metadata"
assert_make_top_level_contains \
	"$PACKAGE_MAKEFILE" \
...
assert_make_block_contains \
	...
	'^[[:space:]]*DEPENDS[[:space:]]*:=[[:space:]]*.*@TARGET_mediatek_mt7622([[:space:]]|$)' \
	"gates the active package metadata on TARGET_mediatek_mt7622"
