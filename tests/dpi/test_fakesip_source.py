...
ARCHIVE = ROOT / "third_party/sources/FakeSIP-Formula-0.9.7.tar.gz"
ARCHIVE_ROOT = "FakeSIP-Formula-0.9.7"
...
def main() -> None:
    ...
    assert "FAKESIP_VERSION:=0.9.7" in wrapper
    assert "$(MAKE) -C $(PKG_BUILD_DIR)/$(1)" in wrapper
    assert "Build/Compile/one,fakesip" in wrapper
    ...
    assert "FakeSIP 0.9.7" in provenance
