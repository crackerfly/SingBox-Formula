...
5. SDK 正常生成 `liquid-formula` 与 `luci-app-liquid-formula` APK。
...
1. Batch 1 为 `liquid-formula-1.5.0-web-upload-batch-1-openwrt-feed.zip`，wrapper prefix 是
   `Liquid-Formula-1.5.0-web-upload-batch-1/`，只包含 `openwrt-feed/` 下的 88 个 tracked files；
2. Batch 2 为 `liquid-formula-1.5.0-web-upload-batch-2-repository.zip`，wrapper prefix 是
   `Liquid-Formula-1.5.0-web-upload-batch-2/`，包含 `openwrt-feed/` 外的 22 个 tracked files；
   它的六个顶层入口是 `.github`、`.gitignore`、`README.md`、`docs`、`momo-template.json` 和
   `tests`。

同时生成 `liquid-formula-1.5.0-complete-source.zip`，prefix 为 `Liquid-Formula-1.5.0/`，包含
