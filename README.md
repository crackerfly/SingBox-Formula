...
Liquid Formula builds the pinned [`sb-sub-c`](#the-sb-sub-c-source) converter source (v0.7.2) as a
...
**Version:** 1.5.0 · **Two packages:** `liquid-formula` (service) + `luci-app-liquid-formula` (UI),
versioned together.
...
Grab two files from the [latest release](../../releases/latest):

1. `liquid-formula_<arch>_openwrt-<version>.apk` (or `.ipk` on 24.10) matching your
   router's CPU architecture — check it with `opkg print-architecture` or
   `apk --print-arch`, or read `DISTRIB_ARCH` from `/etc/openwrt_release`.
2. `luci-app-liquid-formula_all_openwrt-<version>.apk` — the same file works on every
   architecture for that OpenWrt release, because the package is `PKGARCH:=all`.

Install the service package first, then the LuCI app. Verify downloads against
`SHA256SUMS` in the same release.

## Features

- **Subscription → sing-box JSON** conversion driven entirely from LuCI.
- **Two-tab LuCI UI** (`Services → Liquid Formula`): *Overview* (settings, integration, service
...
- **Prebuilt for 43 targets**: every GitHub release ships `liquid-formula` for 21 CPU
  architectures on OpenWrt 24.10 and 22 on 25.12, plus one architecture independent
  `luci-app-liquid-formula` per release. SNAPSHOT is deliberately not built.
- **momo firewall bypass switches**: keeps the FakeHTTP (`0x8000/0x8000`) and FakeSIP
  (`0x10000/0x10000`) marks in momo's `bypass_fwmark` list, so those tools' forged packets
  are not swallowed by momo's transparent proxy. Shown only when momo is installed.
- **Tuning Utility page**: custom LuCI logo/favicon, plus optional kernel network tuning
  (TCP Fast Open, default qdisc, congestion control, SYN backlog) and an irqbalance switch.
  The sysctl values go into a package-owned `/etc/sysctl.d/` drop-in that is rewritten on
  every apply, and the page shows what the kernel is *actually* running, not just what is
  configured.
- **Converter URLs for both scopes**: a loopback URL for sing-box runtimes on this router and a
  LAN URL (derived from the `lan` interface address) for phones, PCs and other devices.
...
- **Subscription format handling**: pick the `User-Agent` your provider expects from a preset list
  (sing-box, mihomo, Clash, v2rayN, Shadowrocket, Surge, Stash, Loon, ...) or type your own. Whatever
  comes back — sing-box JSON, a base64 node list or a plain URI list — is normalised into sing-box
  outbounds automatically, covering ss / vmess / vless / trojan / hysteria2 / tuic / anytls / socks.
...
```sh
apk update
apk add luci-app-liquid-formula
```
...
```sh
apk update
apk upgrade liquid-formula luci-app-liquid-formula
```
...
```sh
apk del luci-app-liquid-formula liquid-formula
```
...
1. Open **Services → Liquid Formula → Overview**.
...
| **Generate config.yaml** | Rebuild `/etc/liquid-formula/config.yaml` from your saved settings. |
...
default template cannot be deleted. Templates live in `/www/liquid-formula/templates/` and are
served to the converter over `http://127.0.0.1/liquid-formula/templates/`.

---

## Configuration reference

UCI config file: `/etc/config/liquid_formula`, section `config global 'main'`.
...
| `user_agent` | `sing-box 1.11.0` | `User-Agent` sent when fetching the subscription. Empty falls back to the converter default. Printable ASCII, max 200 chars. |
...
| `log_file` | `/var/log/liquid-formula/server.log` | Converter log file. |
| `output_config` | `/etc/momo/profiles/config.json` | Where the validated profile is written by *Update output file*. |
| `template_base_url` | `http://127.0.0.1/liquid-formula/templates` | Local URL prefix the converter uses to fetch templates. |
...
```sh
uci set liquid_formula.main.refresh_interval='30'
uci commit liquid_formula
/etc/init.d/liquid-formula restart
```
...
Subscription URL ──▶ sb-sub-c (HTTP :9716) ──▶ converted sing-box JSON
 ...
 (from your UCI)   (/www/liquid-formula/…)             └─▶ written to output_config
...
1. `generate-config.sh` renders `/etc/liquid-formula/config.yaml` from your UCI settings.
...
- Service: `/etc/init.d/liquid-formula` · RPC backend: `/usr/libexec/rpcd/liquid_formula`
- Converter config: `/etc/liquid-formula/config.yaml`
- Helper scripts: `/usr/share/liquid-formula/{generate-config.sh,update.sh,validate-template.sh}`
- Logs: `/var/log/liquid-formula/{update.log,server.log}`
...
That is fine — the converter decodes base64 and plain URI lists on its own and turns them into
sing-box outbounds. No option to toggle.

**Q: My provider says "client version too low", or only ever sends base64.**
Panels such as V2Board / Xboard decide what to send from the `User-Agent`, and reject unknown or
outdated clients outright. Set **Subscription User-Agent** to something your provider recognises;
the field is a combo box with common client strings and also accepts free text. Run
`tools/probe-subscription.sh` on the router to see what each `User-Agent` actually returns before
committing to one.
...
- **Batch 1:** `liquid-formula-1.5.0-web-upload-batch-1-openwrt-feed.zip` contains only
  `openwrt-feed/`, exactly 88 tracked files.
- **Batch 2:** `liquid-formula-1.5.0-web-upload-batch-2-repository.zip` contains every tracked path
...
The separate `liquid-formula-1.5.0-complete-source.zip` contains all 110 tracked files for source
...
3. At the repository root, choose **Add file → Upload files**. For Round 1, open the extracted
   `Liquid-Formula-1.5.0-web-upload-batch-1` wrapper, drag its `openwrt-feed` directory into the
   browser, and commit the upload.
4. Return to the repository root and choose **Add file → Upload files** again. For Round 2, open the
   extracted `Liquid-Formula-1.5.0-web-upload-batch-2` wrapper, drag all six top-level entries
...
`openwrt-feed/liquid-formula/src/`, or **Upload files** may reject them when submitted directly.
...
only for `openwrt-feed/liquid-formula/src/.env` and
`openwrt-feed/liquid-formula/src/.github/workflows/go-release-docker.yml`; the original bytes also
...
(cd openwrt-feed/liquid-formula/src && go test -race ./... && go vet ./...)
```
