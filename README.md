...
1. `singbox-formula_<arch>_openwrt-<version>.apk` (or `.ipk` on 24.10) matching your
...
2. `luci-app-singbox-formula_all_openwrt-<version>.apk` — the same file works on every
...
- **Prebuilt for 43 targets**: every GitHub release ships `singbox-formula` for 21 CPU
  architectures on OpenWrt 24.10 and 22 on 25.12, plus one architecture independent
  `luci-app-singbox-formula` per release. SNAPSHOT is deliberately not built.
...
**Q: My provider needs `flag=singbox` (or another query parameter).**
Put it in the subscription URL itself. The URL is passed through untouched, so
`https://provider.example/sub?token=...&flag=singbox` works as-is. The dedicated toggle was removed
in 1.6.0 because very few providers honour the parameter.
