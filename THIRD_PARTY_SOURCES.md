...
`liquid-formula-dpi` builds two GNU GPLv3 tools from source: both are
...
vendored trees under `openwrt-feed/liquid-formula-dpi/src/` and those archives
are verified in CI before anything is compiled.

## Project-maintained FakeHTTP 0.9.20
...
- Maintained build source: `openwrt-feed/liquid-formula-dpi/src/fakehttp/`
- Corresponding source archive: `third_party/sources/FakeHTTP-Formula-0.9.20.tar.gz`
- Build: compiled by the pinned OpenWrt SDK as part of `liquid-formula-dpi`;
  no precompiled FakeHTTP executable is stored in the repository.

The 0.9.20 delta over upstream 0.9.18 is two fixes. The main one: the TCP data
...
## Project-maintained FakeSIP 0.9.7
...
- Maintained build source: `openwrt-feed/liquid-formula-dpi/src/fakesip/`
- Corresponding source archive:
  `third_party/sources/FakeSIP-Formula-0.9.7.tar.gz`
- Source archive SHA-256:
  `359018c507786d0af70d57e33fe564c3a76f72fe1821fbb6d209ab7cc2652b6c`
- Build: compiled by the pinned OpenWrt SDK as part of `liquid-formula-dpi`;
...
Version 0.9.7 restores the dynamic TTL estimation: fs_srcinfo_put() was
...
adds `-p`/`-P` UDP port filters. Version 0.9.7 retains one known limitation: it
...
`openwrt-feed/liquid-formula-dpi/Makefile`. The reproducible supported path is
...
```sh
make -C openwrt-feed/liquid-formula-dpi/src/fakehttp \
	CC=aarch64-openwrt-linux-musl-gcc VERSION=0.9.20
```
