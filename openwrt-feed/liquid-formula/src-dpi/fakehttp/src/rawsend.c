                             ...
                             struct tcphdr *tcph)
{
    ...
    /*
        Defence in depth: the packet parsers reject doff < 5 already, but this
        subtraction is unsigned, so a malformed header that reached here would
        wrap tcpopt_len to a huge value and scan out of bounds. Report "nothing
        removed", which is exactly how a SYN without a TFO cookie is treated.
    */
