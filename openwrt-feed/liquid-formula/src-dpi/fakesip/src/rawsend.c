                      ...
                      int *modified)
{
    ...
    *modified = 0;
    ...
    if (g_ctx.use_ipv4 && ethertype == ETHERTYPE_IP) {
        ...
        /* 这里原本少了实参: %04x 会去读一个不存在的 vararg(UB)。 */
        E("ERROR: unknown ethertype 0x%04x", (unsigned int) ethertype);
        return -1;
    }
    ...
    if (sll->sll_pkttype == PACKET_HOST) {
        ...
        */
        ...
        /*
            Record what we learned about this peer before any direction gate:
            its arriving TTL and MAC. The outbound branch looks the same peer
            up with fs_srcinfo_get() to size its decoy TTL against the real
            path.

            This must sit above the `!g_ctx.outbound` return below. With the
            default direction the init script passes -0, which sets
            g_ctx.inbound and leaves g_ctx.outbound clear, so anything placed
            after that gate never runs and every decoy falls back to the fixed
            initial TTL. Observing a packet is not an injection, so gating it
            on the injection direction was wrong to begin with.
        */
