...
type SubscriptionConfig struct {
	URL             string `yaml:"url"`
	UserAgent       string `yaml:"user_agent"`       // 拉取订阅时使用的 User-Agent，留空则用内置默认值
...
func (c *Config) Validate() error {
	...
	// User-Agent 会原样进入 HTTP 头，必须是可打印 ASCII，否则 Transport 会直接报错。
	if ua := c.Subscription.UserAgent; ua != "" {
		if len(ua) > 200 {
			return fmt.Errorf("subscription user_agent is too long (%d > 200)", len(ua))
		}
		for _, r := range ua {
			if r < 0x20 || r > 0x7e {
				return fmt.Errorf("subscription user_agent must be printable ASCII")
			}
		}
	}
...
// DefaultUserAgent 是未显式配置 user_agent 时使用的默认值。
// 机场面板（V2Board / Xboard 等）大多按 User-Agent 决定下发格式与版本门槛，
// 通用浏览器 UA 常常触发“客户端版本过低”或只拿到 base64 列表。
const DefaultUserAgent = "sing-box 1.11.0"

// GetUserAgent 获取拉取订阅使用的 User-Agent。
func (c *Config) GetUserAgent() string {
	if c != nil {
		if ua := strings.TrimSpace(c.Subscription.UserAgent); ua != "" {
			return ua
		}
	}
