...
refresh_converter() {
	...
	case "$code" in
		401)
			log "refresh rejected with HTTP 401: the converter password does not match the UCI password; run Save & Apply to regenerate /etc/singbox-formula/config.yaml" || true
			;;
		*)
