...
var callStatus = rpc.declare({ object: 'singbox-subscribe-convert', method: 'status', expect: { '': {} } });
var callAction = rpc.declare({ object: 'singbox-subscribe-convert', method: 'action', params: [ 'name' ], expect: { '': {} } });
var callListTemplates = rpc.declare({ object: 'singbox-subscribe-convert', method: 'list_templates', expect: { '': {} } });
var callReadTemplate = rpc.declare({ object: 'singbox-subscribe-convert', method: 'read_template', params: [ 'id', 'file' ], expect: { '': {} } });
var callWriteTemplate = rpc.declare({ object: 'singbox-subscribe-convert', method: 'write_template', params: [ 'id', 'name', 'file', 'no_node', 'enabled', 'content' ], expect: { '': {} } });
var callDeleteTemplate = rpc.declare({ object: 'singbox-subscribe-convert', method: 'delete_template', params: [ 'id' ], expect: { '': {} } });
...
return view.extend({
	load: function() {
		return Promise.all([
			uci.load('singbox_subscribe_convert'),
			callStatus(),
			callListTemplates()
		]);
	},
	...
	render: function(data) {
		...
		m = new form.Map('singbox_subscribe_convert', _('Sing-box Subscribe Convert'),
		...
		o.default = 'http://127.0.0.1/singbox-subscribe-convert/templates';
	...
	saveTemplate: function() {
		...
		return callWriteTemplate(id, name, file, noNode, enabled, content).then(function(res) {
			if (res.error)
				return ui.addNotification(null, E('p', {}, res.error));
			ui.addNotification(null, E('p', {}, _('Template saved. Reload this page to refresh the list.')));
		}).catch(ui.addNotification.bind(ui, null));
	},
