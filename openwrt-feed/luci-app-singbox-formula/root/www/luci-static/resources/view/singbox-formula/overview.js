...
return view.extend({
	...
	render: function(data) {
		...
		return m.render().then(L.bind(function(formEl) {
			// Auto-refresh the status card so state (e.g. after Save & Apply, while
			// the service is still coming up) updates without a manual page reload.
			// Skipped while an action is mid-flight so it can't interrupt a spinner.
	...
	// After Save & Apply commits the config, reconcile the running state to the
	// 'enabled' flag by running a normal init start/stop via rpcd. This is
	// reliable, unlike starting from inside the procd reload trigger. The flag
	// itself is written by the form; the rc.d symlink for autostart-on-boot is
	// created at install, so boot() honors the flag regardless. Then refresh the
	// status card.
	handleSaveApply: function(ev, mode) {
		var self = this;
		var parent = (view.prototype && typeof view.prototype.handleSaveApply == 'function')
			? view.prototype.handleSaveApply.call(this, ev, mode)
			: this.handleSave(ev).then(function() { return ui.changes.apply(mode == '0'); });
		return Promise.resolve(parent).then(function() {
			var enabled = (uci.get('singbox_formula', 'main', 'enabled') == '1');
			return callAction(enabled ? 'start' : 'stop').then(function() {
				return self.reloadStatus();
			}).catch(function() { /* ignore reconcile errors; status still polls */ });
		});
	},
	...
	doAction: function(name, successMsg, btn) {
		...
		var done = function() { self._busy = false; if (btn) { btn.classList.remove('spinning'); btn.disabled = false; } };
	...
	statusChildren: function(status) {
		...
		return [
			...
			E('p', { 'class': 'cbi-value-description' }, [
				_('Starting and stopping the converter is controlled by the Enable converter service switch in Basic Settings — tick or untick it and Save & Apply. Use Restart converter to restart the running process. '),
				_('Refresh, Check and Update need the converter running — they will start it automatically if needed. Update output file only writes the generated JSON; it does not restart sing-box (manage that from your sing-box runtime). This status refreshes automatically.')
			]),
			E('h4', {}, _('Health Check')),
